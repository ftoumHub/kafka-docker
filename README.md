...
```sh
docker build . -t ggn/kafka:2.7.0
```
...
```sh
docker run --rm --name kafka -it ggn/kafka:2.7.0 bash
```
```
ls -l /kafka/bin/
...
Let's build a Zookeeper image. The Apache folks have made it easy to start a Zookeeper
instance the same way as the Kafka instance by simply running the `start-zookeeper.sh` script.

```
cd ./zookeeper
docker build . -t ggn/zookeeper:2.7.0
cd ..
```
